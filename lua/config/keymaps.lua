...
keymap(
  ...
  { desc = "[F]ind using [G]rep in current project" }
)
keymap(
  "n",
  "<leader>/",
...
keymap(
  ...
  { desc = "Grep Words under cursor" }
)
keymap(
  "v",
  "<leader>fw",
  require("fzf-lua").grep_visual,
  { desc = "Grep Words selected using Visual Mode" }
)
...
keymap(
  ...
  { desc = "[Find] in [R]egisters" }
)
keymap(
  "n",
  "<leader>fq",
  require("fzf-lua").grep_quickfix,
  { desc = "[Find] in [Q]uickfix list" }
)
...
keymap(
  ...
  { desc = "Show [G]it [BR]anches" }
)
keymap(
  "n",
  "<leader>gbl",
  require("fzf-lua").git_blame,
  { desc = "Show [G]it [BL]lame for buffer" }
)
keymap(
  "n",
  "<leader>gcp",
  require("fzf-lua").git_commits,
  { desc = "Show [G]it [C]ommits in [P]roject" }
)
keymap(
  "n",
  "<leader>gcb",
  require("fzf-lua").git_bcommits,
  { desc = "Show [G]it [C]ommits in [B]uffer" }
)
keymap(
  "n",
  "<leader>gst",
  require("fzf-lua").git_status,
  { desc = "Show [G]it [ST]atus" }
)
keymap(
  "n",
  "<leader>gdf",
  require("fzf-lua").git_diff,
  { desc = "Show [G]it [D]if[F]" }
)
...
keymap(
  ...
  { desc = "[L]SP [D]e[c]larations" }
)
keymap(
  "n",
  "<leader>limp",
  require("fzf-lua").lsp_implementations,
  { desc = "[L]SP [IMP]lementations" }
)
keymap(
  "n",
  "<leader>lds",
  require("fzf-lua").lsp_document_symbols,
  { desc = "[L]SP [D]ocument [S]ymbols" }
)
keymap(
  "n",
  "<leader>lws",
...
keymap(
  ...
  { desc = "[L]SP [D]ocument [D]iagnostics" }
)
keymap(
  "n",
  "<leader>lwd",
  require("fzf-lua").lsp_workspace_diagnostics,
  { desc = "[L]SP [W]orkspace [D]iagnostics" }
)
keymap(
  "n",
  "<leader>lca",
